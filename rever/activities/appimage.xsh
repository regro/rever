...
class AppImage(Activity):
    ...
    def _func(self, template='$VERSION', python_version=None):
        ...
        if platform.system() == 'Linux':
            ...
            docker run -v @(path):/dir --rm -e GID=@$(id -g) -e UID=@$(id -u) python:3.11-slim-bullseye bash -c @(f'addgroup --gid $GID user && adduser --disabled-password --gecos "" --uid $UID --gid $GID user && apt update && apt install -y git file gpg && pip install git+https://github.com/niess/python-appimage && chown -R user:user /dir && su - user -c "cd /dir && python -m python_appimage build app {" ".join(python_ver)} {self.appimage_descr_dir}"')
